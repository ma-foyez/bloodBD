<div x-data="{ open: @entangle('isModalOpen'), isDeleteModalOpen: @entangle('isDeleteModalOpen') }">
    <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
            <h2 class="text-xl font-bold">Unions</h2>
            <div class="flex items-center gap-3">
                <x-inputs.input type="text" wire:model.live="search" placeholder="Search Unions..." />
        ...
        <div class="table-responsive">
            <table class="table">
                <thead class="table-thead">
                    <tr class="table-tr">
                        <th class="table-thead-th">Area</th>
                ...
                <tbody>
                    @foreach ($unions as $union)
                        <tr class="table-tr">
                            <td class="table-td">{{ $union->area->name ?? 'N/A' }}</td>
                            <td class="table-td">{{ $union->name }}</td>
                            <td class="table-td">{{ $union->bn_name }}</td>
                            <td class="table-td text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <x-buttons.button wire:click="edit({{ $union->id }})" variant="info"
                                        size="sm">Edit</x-buttons.button>
                                    <x-buttons.button wire:click="confirmDelete({{ $union->id }})" variant="danger"
    ...
    <x-modal.modal wire:model="isModalOpen">
        <x-slot name="header">
            {{ $unionId ? 'Edit Union' : 'Create Union' }}
        </x-slot>

        <div class="space-y-4">
            <div wire:key="division-select-{{ $unionId ?? 'create' }}">
            ...
            <div wire:key="area-select-{{ $district_id }}">
                <x-inputs.combobox name="area_id" label="Area" :options="$areas ? collect($areas)->pluck('name', 'id') : []" :selected="$area_id" searchable="true"
                    x-on:combobox-change="$wire.set('area_id', $event.detail.value)" required
    ...
    <x-modals.confirm-delete wire:model.live="isDeleteModalOpen" modalTrigger="isDeleteModalOpen" title="Delete Union"
        content="Are you sure you want to delete this Union? This action cannot be undone." wireClick="delete" />
</div>
