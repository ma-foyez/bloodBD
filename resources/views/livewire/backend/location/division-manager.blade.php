<div x-data="{ open: @entangle('isModalOpen'), isDeleteModalOpen: @entangle('isDeleteModalOpen') }">
    <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
            <h2 class="text-xl font-bold">Divisions</h2>
            <div class="flex items-center gap-3">
                <x-inputs.input type="text" wire:model.live="search" placeholder="Search Divisions..." />
            ...
            <table class="table">
                <thead class="table-thead">
                    <tr class="table-tr">
                        ...
                        <th class="table-thead-th">URL</th>
                ...
                <tbody>
                    @foreach ($divisions as $division)
                        <tr class="table-tr">
                            ...
                            <td class="table-td">{{ $division->bn_name }}</td>
                            <td class="table-td">{{ $division->url }}</td>
                            <td class="table-td text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <x-buttons.button wire:click="edit({{ $division->id }})" variant="info"
                                        size="sm">
                                        Edit
                                    </x-buttons.button>
                                    <x-buttons.button wire:click="confirmDelete({{ $division->id }})" variant="danger"
                                        size="sm">
                                        Delete
                                    </x-buttons.button>
                                </div>
    ...
    <x-modal.modal wire:model="isModalOpen">
        ...
        <x-slot name="footer">
            <x-buttons.button wire:click="closeModal" variant="secondary">
                Cancel
            </x-buttons.button>
            <x-buttons.button wire:click="store" variant="primary">
                Save
            </x-buttons.button>
        </x-slot>
    </x-modal.modal>
    ...
    <x-modals.confirm-delete wire:model.live="isDeleteModalOpen" modalTrigger="isDeleteModalOpen"
        title="Delete Division" content="Are you sure you want to delete this Division? This action cannot be undone."
        wireClick="delete" />
</div>
